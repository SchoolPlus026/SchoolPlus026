...
export default function RegisterSchool() {
  ...
  useEffect(() => {
    supabase.from('platform_settings').select('app_name').single()
  ...
  const validateStep = () => {
    if (step === 0) {
      ...
      if (!/^[A-Za-z0-9]+$/.test(form.school_code)) return 'School code can only contain letters and numbers (no spaces).';
    ...
    if (step === 1) {
      ...
      if (!/^[a-z0-9_]+$/.test(form.admin_username)) return 'Username can only contain lowercase letters, numbers, and underscores.';
    }
    if (step === 2) {
    ...
    try {
      ...
      if (insertError) {
        if (insertError.code === '23505') {
          throw new Error(`School code "${form.school_code.toUpperCase()}" already has a pending registration. Please wait for review or choose a different code.`);
        }
      ...
      <div style={pageStyle}>
        <div style={cardStyle}>
          <div style={{ textAlign: 'center', padding: '40px 20px' }}>
            ...
            <p style={{ color: '#94a3b8', fontSize: 14, lineHeight: 1.7, marginBottom: 24, maxWidth: 380, margin: '0 auto 24px' }}>
  ...
  const Field = ({ label, icon: Icon, children }) => (
    <div style={{ marginBottom: 16 }}>
      <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>
        {Icon && <Icon size={12} style={{ display: 'inline', marginRight: 4 }} />}{label}
      </label>
      {children}
    </div>
  );
    ...
    <div style={pageStyle}>
      <div style={cardStyle}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: 13, margin: 0 }}>Join the <strong style={{ color: '#c7d2fe' }}>{appName}</strong> network in minutes</p>
        </div>
        ...
        {/* Error Banner */}
        {error && (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '10px 14px', marginBottom: 16, color: '#f87171', fontSize: 13 }}>
        ...
        <form onSubmit={step === STEPS.length - 1 ? handleSubmit : (e) => { e.preventDefault(); next(); }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div style={{ gridColumn: '1 / -1' }}>
                  <Field label="School Name *" icon={School}>
                    <input required style={inputStyle} placeholder="e.g. Delhi Public School, Sector 14"
                      value={form.school_name} onChange={set('school_name')} />
                  </Field>
                </div>
                <Field label="School Code * (unique ID)" icon={Hash}>
                  <input required style={{ ...inputStyle, textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.1em' }}
                    placeholder="e.g. DPS14" maxLength={10}
                    value={form.school_code}
                    onChange={(e) => setForm(f => ({ ...f, school_code: e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '') }))} />
                  <div style={{ fontSize: 10, color: '#64748b', marginTop: 4 }}>Letters and numbers only. Students will use this to log in.</div>
                </Field>
                ...
                <Field label="Approx. Student Strength">
                  <input type="number" min="1" style={inputStyle} placeholder="e.g. 800"
                    value={form.student_strength} onChange={set('student_strength')} />
                </Field>
              </div>
            </div>
          )}
          ...
          {step === 1 && (
            <div>
              <div style={{ background: 'rgba(79,70,229,0.08)', border: '1px solid rgba(79,70,229,0.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: '#a5b4fc' }}>
                ℹ️ These details will become the <strong>Primary Admin</strong> account for your school. Credentials will be emailed to you after approval.
              </div>
              <Field label="Admin Full Name *" icon={User}>
                <input required style={inputStyle} placeholder="e.g. Ravi Kumar Sharma"
                  value={form.admin_name} onChange={set('admin_name')} />
              </Field>
              <Field label="Admin Email * (credentials will be sent here)" icon={Mail}>
                <input required type="email" style={inputStyle} placeholder="e.g. principal@yourschool.edu"
                  value={form.admin_email} onChange={set('admin_email')} />
              </Field>
              <Field label="Admin Phone (optional)" icon={Phone}>
                <input type="tel" style={inputStyle} placeholder="e.g. 9876543210"
                  value={form.admin_phone} onChange={set('admin_phone')} />
              </Field>
              <Field label="Admin Username * (for login)" icon={User}>
                <input required style={{ ...inputStyle, fontFamily: 'monospace' }}
                  placeholder="e.g. admin_dps14"
                  value={form.admin_username}
                  onChange={(e) => setForm(f => ({ ...f, admin_username: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '') }))} />
                <div style={{ fontSize: 10, color: '#64748b', marginTop: 4 }}>Only lowercase letters, numbers, underscores. Min 4 characters.</div>
              </Field>
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              <Field label="Starting Plan" icon={BookOpen}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 4 }}>
                  {[
                    ...
                    <div
                      key={plan.value}
                      onClick={() => setForm(f => ({ ...f, plan_type: plan.value }))}
                      style={{
                        border: `2px solid ${form.plan_type === plan.value ? plan.color : 'rgba(255,255,255,0.1)'}`,
                        borderRadius: 12, padding: '14px', cursor: 'pointer',
                        background: form.plan_type === plan.value ? `rgba(${plan.value === 'trial' ? '74,222,128' : '148,163,184'},0.08)` : 'rgba(255,255,255,0.03)',
              ...
              {/* Summary Card */}
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '16px', marginBottom: 20, fontSize: 12 }}>
                ...
                {[
                  ...
                  ['Code', form.school_code || '—'],
                  ['State', form.state || '—'],
                  ['Admin', form.admin_name || '—'],
              ...
              {/* T&C */}
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer', padding: '14px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: `1px solid ${form.terms_accepted ? 'rgba(79,70,229,0.4)' : 'rgba(255,255,255,0.1)'}`, transition: 'border-color 0.2s' }}>
                <input
                  type="checkbox"
                  checked={form.terms_accepted}
                  onChange={set('terms_accepted')}
                  style={{ width: 18, height: 18, accentColor: '#4f46e5', marginTop: 2, flexShrink: 0 }}
                />
                <span style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.6 }}>
                  I agree to the <strong style={{ color: '#a5b4fc' }}>Terms & Conditions</strong> and <strong style={{ color: '#a5b4fc' }}>Privacy Policy</strong> of {appName}. I confirm that the information provided is accurate and I am authorized to register this institution.
                </span>
              </label>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
            {step > 0 && (
              <button type="button" onClick={prev} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#94a3b8', fontWeight: 700, fontSize: 13, padding: '12px', borderRadius: 12, cursor: 'pointer' }}>
                <ChevronLeft size={16} /> Back
              </button>
            )}
            <button
              type="submit"
              disabled={loading || (step === 2 && !form.terms_accepted)}
              style={{ flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', fontWeight: 800, fontSize: 14, padding: '13px', borderRadius: 12, cursor: loading ? 'not-allowed' : 'pointer', border: 'none', opacity: (step === 2 && !form.terms_accepted) ? 0.5 : 1, transition: 'opacity 0.2s' }}
            >
