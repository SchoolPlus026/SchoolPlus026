...
import { LayoutDashboard, Calendar, Bell, Clock, DollarSign, User, LogOut, Menu, X, CheckSquare } from 'lucide-react';
...
export default function StudentLayout() {
  ...
  const menuItems = [
    ...
    { name: 'My Attendance', path: '/student/attendance', icon: <CheckSquare size={20} /> },
    { name: 'My Fees', path: '/student/fees', icon: <DollarSign size={20} /> },
    { name: 'Timetable', path: '/student/timetable', icon: <Clock size={20} /> },
    ...
    <div className="flex h-screen bg-background text-text overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-surface border-r border-glass transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out`}>
        <div className="flex items-center justify-between h-16 px-4 border-b border-glass">
          <div className="flex items-center gap-3">
            {schoolSettings?.logo_url ? (
              ...
              <div className="w-8 h-8 bg-[#0d9488] rounded-lg flex items-center justify-center font-bold text-white shadow-lg">❖</div>
            )}
        ...
        <nav className="p-4 space-y-1.5 overflow-y-auto h-[calc(100vh-4rem)]">
          <p className="px-3 text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-3">Student Panel</p>
          {menuItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 ${
                  isActive ? 'bg-[#0d9488]/10 text-[#0d9488] font-semibold shadow-inner' : 'text-slate-400 hover:bg-glass hover:text-slate-100'
                }`
              }
            >
              {item.icon}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 bg-surface/40 border-b border-glass backdrop-blur-lg">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold hidden sm:block text-white">Student Portal</h1>
          </div>
          <div className="flex items-center gap-4">
